...
- SG login = **email**. Channels from doc fields: `emp:`, `email:`, `cus:`, **`route:`** (at least one on users / workorders* / orders / notes). Never channel by `type`. Session via `POST /_session`. [docs/AUTH.md](docs/AUTH.md)
...
- **Live queries:** Today jobs + orders use `Query.addChangeListener` (`src/db/liveQuery.ts`, `watchTodayWork`, `watchTodayOrders`). The listener’s first snapshot is the initial paint — do not `execute()` the same SQL++ again on attach. Pages 2+ stay one-shot `execute()`. Native pull-to-refresh applies inbound kit on on-screen outbound copies; the live list is the source of truth. Skip the jobs watcher in sales mode and the orders watcher in assets mode.
